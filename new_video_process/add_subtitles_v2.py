...
import lpc_sound_cluster as sound_cluster
...
def insert_text():
    ...
    distinguish_results = sound_cluster.Speaker_cluster(K) #K is the cluster number of face cluster
    ...
    cnt_frames= 0
    id, x, y = 0, 0, 0
    group_to_color = [(255, 0, 0), (0, 255, 255), (240, 32, 160), (0, 165, 255), (179, 52, 255), (0, 0, 255)]

    data, old_id = [], []
    OPEN_CLOSE = [[False]*20 for i in range(K)]
    
    # 設定文字資料
    fontpath = "font/simsun.ttc"
    FontSize = 32
    if FontSize > w/25: FontSize = 16
    ...
    with mp_face_mesh.FaceMesh(max_num_faces = 3, refine_landmarks = True, min_detection_confidence = 0.5, min_tracking_confidence = 0.5) as face_mesh:
        ...
        while True:
            ...
            if results.multi_face_landmarks:
                ...
                flag_InText = now >= subtitle_inf.iloc[id]['起始時間'] and now <= subtitle_inf.iloc[id]['結束時間']
                ...
                # 當前有字幕
                if flag_InText: flag_NewText = True
                ...
                for face_data in results.multi_face_landmarks:
                    ...
                    cv2.rectangle(img, (int(face_data.landmark[109].x*w), int(face_data.landmark[109].y*h)), (int(face_data.landmark[336].x*w), int(face_data.landmark[336].y*h)), group_to_color[the_group], -1)
            ...
            while i < len(data):
                ...
                if flag_NewFace and data[i][4] == 0:
                    ...
                    NewData = [True, owner, data[i][0][0], data[i][1][1]]
                ...
                if data[i][5] == -1:
                    ...
                    zeros_mask1 = cv2.rectangle(zeros, (bbox[0], bbox[1]), (bbox[2], bbox[3]), color=group_to_color[data[i][5]], thickness=-1)
                zeros_mask = np.array(zeros_mask1)
                img = cv2.addWeighted(img, 1,  zeros_mask, 0.6, 1)
